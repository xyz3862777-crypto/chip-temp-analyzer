...
const M = require('./ideal-model.js');
...
const effectivePath=M.path({...pathInput,deltaV:r.effectiveDeltaVP});
close(r.ac,960*effectivePath.averageMW); close(r.fixedDC,54);
...
// Slide trend: DBC/SRE raise Source DC while reducing effective ΔV and AC.
const noBoost=M.calculate({...d,dbcEnabled:false});
assert.ok(r.ac < noBoost.ac && r.sourceDC > noBoost.sourceDC);
const sre=M.calculate({...d,sreEnabled:true,sreDeltaReduction:12,sreBiasMultiplier:1.25});
assert.ok(sre.ac < noBoost.ac && sre.sourceDC > noBoost.sourceDC);
const heavy=M.calculate({...d,panelR:12000,panelC:800});
const light=M.calculate({...d,panelR:1500,panelC:100});
assert.ok(heavy.acReductionPercent > light.acReductionPercent);
const pwrcLow=M.calculate({...d,pwrc:7});
assert.ok(pwrcLow.effectiveDeltaVP < r.effectiveDeltaVP && pwrcLow.sourceDC < r.sourceDC);
